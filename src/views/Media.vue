    ...
    <dialog-modal :show="shownModal">
        ...
        <template #content>
          ...
          <div class="mt-4 w-2/3">
            ...
            <div class="pb-4 mt-1 flex items-center space-x-2">
              <input v-model="api" type="text" @blur="callRemoteApi()" class="shadow-sm focus:ring-blue-500 focus:border-blue-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md"/>
              <svg v-if="1 === showIcon" class="animate-spin h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>

              <svg v-if="2 === showIcon" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
              
              <svg v-if="3 === showIcon" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default {
  ...
  mounted() {
    ipcRenderer.on('participantFileSelected', (event, args) => {
      this.$store.commit('setStringInput', args.contents)
      this.$store.dispatch('updateContestants');

      new Notification('Archivo subido', { body: 'La carga del archivo se realizó correctamente' });
    });

    ipcRenderer.on('participantApiLoaded', (event, args) => {
      this.$store.commit('setStringInput', args.contents)
      this.$store.dispatch('updateContestants');

      this.showIcon = icons.success;

      new Notification('REMOTE API', { body: 'Los datos se cargaron desde la API que se proporcionó.' });
    })

    ipcRenderer.on('participantApiFailed', (event, args) => {
      this.message = args.contents;
      this.showIcon = icons.error;
    })
  ...
  methods: {
    openFile() {
      ipcRenderer.send('openParticipantsFile');
    },

    callRemoteApi() {
      this.message = null;
      this.showIcon = icons.loading;

      ipcRenderer.send('remoteApiRequest', {url: this.api});
    },
